#ifndef _GET_RX_LO_FREQ_RANGE_HPP_
#define _GET_RX_LO_FREQ_RANGE_HPP_

#include "../../API/mex_object_runner.hpp"

using matlab::data::StructArray;

//virtual freq_range_t 	get_rx_lo_freq_range (const std::string &name, size_t chan=0)
class GetRxLoFreqRange : public MexObjectRunner<STRING_REQUIRED, INT64> {
protected:
    virtual void funcImpl(Uhd_ptr uhd, ArgumentList outputs, ArgumentList inputs, matlabEngine engine) override {
		const std::string name = inputs[0][0][0];
        if(inputs.size() == 1) {
            const uhd::freq_range_t freqRange = uhd->getUhd()->get_rx_lo_freq_range(name);
            outputs[0] = getOutputStruct(freqRange);
        }
        else {
			const size_t chan{ inputs[1][0][0] };
            const uhd::freq_range_t freqRange = uhd->getUhd()->get_rx_lo_freq_range(name, chan);
            outputs[0] = getOutputStruct(freqRange);
        } 
    }
