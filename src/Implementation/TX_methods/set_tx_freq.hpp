#ifndef _SET_TX_FREQ_HPP_
#define _SET_TX_FREQ_HPP_
...
//virtual tune_result_t 	set_tx_freq (const tune_request_t &tune_request, size_t chan=0)
class SetTxFreq : public MexObjectRunner<STRUCT_REQUIRED, INT64> {
protected:
    virtual void funcImpl(Uhd_ptr uhd, ArgumentList outputs, ArgumentList inputs, matlabEngine engine) override {
		...
		if (inputs.size() == 1) {
			tune_res = uhd->getUhd()->set_tx_freq(tune_req);
		}
		else {
			const size_t chan{ inputs[1][0][0] };
			tune_res = uhd->getUhd()->set_tx_freq(tune_req, chan);
		}
