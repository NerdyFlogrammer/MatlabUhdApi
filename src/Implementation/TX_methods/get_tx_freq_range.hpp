#ifndef _GET_TX_FREQ_RANGE_HPP_
#define _GET_TX_FREQ_RANGE_HPP_
...
class GetTxFreqRange : public MexObjectRunner<INT64> {
protected:
    virtual void funcImpl(Uhd_ptr uhd, ArgumentList outputs, ArgumentList inputs, matlabEngine engine) override {
		if (inputs.size() == 0) {
			const uhd::meta_range_t freqRange = uhd->getUhd()->get_tx_freq_range();
			outputs[0] = getOutputStruct(freqRange);
		}
		else {
			const size_t chan{ inputs[0][0][0] };
			const uhd::meta_range_t freqRange = uhd->getUhd()->get_tx_freq_range(chan);
			outputs[0] = getOutputStruct(freqRange);
		}
	}
