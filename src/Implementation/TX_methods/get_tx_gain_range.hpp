#ifndef _GET_TX_GAIN_RANGE_HPP_
#define _GET_TX_GAIN_RANGE_HPP_
...
class GetTxGainRange : public MexObjectRunner<STRING_REQUIRED, INT64> {
protected:
	virtual void funcImpl(Uhd_ptr uhd, ArgumentList outputs, ArgumentList inputs, matlabEngine engine) override {
		const std::string name = inputs[0][0][0];
		if (inputs.size() == 1) {
			const uhd::gain_range_t gainRange = uhd->getUhd()->get_tx_gain_range(name);
			outputs[0] = getOutputStruct(gainRange);
		}
		else {
			const size_t chan{ inputs[0][0][0] };
			const uhd::gain_range_t gainRange = uhd->getUhd()->get_tx_gain_range(name, chan);
			outputs[0] = getOutputStruct(gainRange);
		}
	}
