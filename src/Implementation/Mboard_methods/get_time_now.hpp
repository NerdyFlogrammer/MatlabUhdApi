#ifndef _GET_TIME_NOW_HPP_
#define _GET_TIME_NOW_HPP_
...
class GetTimeNow : public MexObjectRunner<INT64> {
protected:
	virtual void funcImpl(Uhd_ptr uhd, ArgumentList outputs, ArgumentList inputs, matlabEngine engine) override {
		if (inputs.size() == 0) {
			const uhd::time_spec_t& time = uhd->getUhd()->get_time_now();
			outputs[0] = getOutputStruct(time);
		}
		else {
			const size_t mboard{ inputs[0][0][0] };
			const uhd::time_spec_t& time = uhd->getUhd()->get_time_now(mboard);
			outputs[0] = getOutputStruct(time);;
		}
	}
