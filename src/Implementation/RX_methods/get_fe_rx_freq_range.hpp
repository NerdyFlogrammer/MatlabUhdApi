#ifndef _GET_FE_RX_FREQ_RANGE_HPP_
#define _GET_FE_RX_FREQ_RANGE_HPP_

#include "../../API/mex_object_runner.hpp"

using matlab::data::StructArray;

//virtual freq_range_t 	get_fe_rx_freq_range (size_t chan=0)
class GetFeRxFreqRange : public MexObjectRunner<INT64> {
protected:
    virtual void funcImpl(Uhd_ptr uhd, ArgumentList outputs, ArgumentList inputs, matlabEngine engine) override {    
        if(inputs.size() == 0) {
            const uhd::freq_range_t freqRange = uhd->getUhd()->get_fe_rx_freq_range();
            outputs[0] = getOutputStruct(freqRange);
        }
        else {
			const size_t chan{ inputs[0][0][0] };
            const uhd::freq_range_t freqRange = uhd->getUhd()->get_fe_rx_freq_range(chan);
            outputs[0] = getOutputStruct(freqRange);
        } 
    }
