#ifndef _GET_RX_BANDWIDTH_RANGE_HPP_
#define _GET_RX_BANDWIDTH_RANGE_HPP_

#include "../../API/mex_object_runner.hpp"

//virtual meta_range_t 	get_rx_bandwidth_range (size_t chan=0)
class GetRxBandwidthRange : public MexObjectRunner<INT64> {
protected:
    virtual void funcImpl(Uhd_ptr uhd, ArgumentList outputs, ArgumentList inputs, matlabEngine engine) override {
		if (inputs.size() == 0) {
			const uhd::meta_range_t bandwidthRange = uhd->getUhd()->get_rx_bandwidth_range();
			outputs[0] = getOutputStruct(bandwidthRange);
		}
		else {
			const size_t chan{ inputs[0][0][0] };
			const uhd::meta_range_t bandwidthRange = uhd->getUhd()->get_rx_bandwidth_range(chan);
			outputs[0] = getOutputStruct(bandwidthRange);
		}
	}
